...
cp .env.example .env
# Edit .env to add: WHATSAPP_TOKEN, SMS_API_KEY, GOOGLE_CLIENT_ID
...
The platform features an automated, zero-configuration translation engine. Every alert generated by the system is seamlessly translated into **Hindi** and **Marathi** using `deep-translator` (Google Translate API) before dispatch. This ensures that facility operators and rural farmers receive critical, time-sensitive information in their native language without requiring any third-party API keys or billing setups.
